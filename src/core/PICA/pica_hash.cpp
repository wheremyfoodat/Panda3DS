...
PICAHash::HashType PICAHash::computeHash(const void* buf, std::size_t len) {
#ifdef PANDA3DS_PICA_CITYHASH
...
PICAShader::Hash PICAShader::getCodeHash() {
	...
	if (codeHashDirty) {
		codeHashDirty = false;
		lastCodeHash = PICAHash::computeHash(&loadedShader[0], loadedShader.size() * sizeof(loadedShader[0]));
	}
...
PICAShader::Hash PICAShader::getOpdescHash() {
	...
	if (opdescHashDirty) {
		...
		lastOpdescHash = PICAHash::computeHash(&operandDescriptors[0], operandDescriptors.size() * sizeof(operandDescriptors[0]));
	}
