...
void* MemoryArena::FindBaseAddressForMapping(size_t size)
{
  ...
  if (!base_address)
  {
    printf("Failed to get base address for memory mapping of size %zu", size);
    return nullptr;
  }
...
bool MemoryArena::Create(size_t size, bool writable, bool executable)
{
...
#ifndef _UWP
  m_file_handle = CreateFileMappingA(INVALID_HANDLE_VALUE, nullptr, protect, uint32_t(size >> 32), uint32_t(size),
  ...
  if (!m_file_handle)
  {
    printf("CreateFileMapping failed: %u", GetLastError());
    return false;
  }
  ...
  if (m_shmem_fd < 0)
  {
    printf("AshmemCreateFileMapping failed: %d %d", m_shmem_fd, errno);
    return false;
  }
  ...
  if (m_shmem_fd < 0)
  {
    printf("shm_open failed: %d", errno);
    return false;
  }
  ...
  {
    printf("ftruncate64(%zu) failed: %d", size, errno);
    return false;
  }
  ...
  {
    printf("ftruncate(%zu) failed: %d", size, errno);
    return false;
  }
...
#ifndef _UWP
  base_pointer =
    MapViewOfFileEx(m_file_handle, desired_access, uint32_t(offset >> 32), uint32_t(offset), size, fixed_address);
#else
...
bool MemoryArena::ReleaseViewPtr(void* address, size_t size)
{
  ...
  if (!result)
  {
    printf("Failed to unmap previously-created view at %p", address);
    return false;
  }
  ...
  if (!(prev_count > 0)) {
      printf("Error unmapping previously-created view");
      std::abort();
  }
  return true;
}
...
bool MemoryArena::ReleaseReservedPtr(void* address, size_t size)
{
  ...
  if (!result)
  {
    printf("Failed to release previously-created view at %p", address);
    return false;
  }

  const size_t prev_count = m_num_views.fetch_sub(1);
  if (!(prev_count > 0)) {
      printf("Error releasing previously-created memory view");
      std::abort();
  }
  return true;
}
  ...
  if (m_parent)
  {
    if (m_arena_offset != RESERVED_REGION_OFFSET)
    {
      if (m_writable && !m_parent->FlushViewPtr(m_base_pointer, m_mapping_size)) {
        printf("Failed to flush previously-created view");
        std::abort();
      }
      if (!m_parent->ReleaseViewPtr(m_base_pointer, m_mapping_size)) {
        printf("Failed to unmap previously-created view");
        std::abort();
      }
    }
    else
    {
      if (!m_parent->ReleaseReservedPtr(m_base_pointer, m_mapping_size)) {
        printf("Failed to release previously-created view");
        std::abort();
      }
    }
  }
